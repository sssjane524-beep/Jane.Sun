(() => {
  ...
  if (finePointer.matches) {
    ...
    function paintCursor(now) {
      ...
      // The light exists only while the pointer is moving. The slower follower
      // leaves a visible gap behind the native cursor, then clears promptly.
      const fade = Math.max(0, Math.min(1, (190 - (now - lastMove)) / 120));
      if (!fade) { trail.length = 0; ctx.clearRect(0, 0, width, height); return; }
      follower.x += (target.x - follower.x) * .11;
      follower.y += (target.y - follower.y) * .11;
      trail.unshift({ x: follower.x, y: follower.y });
      if (trail.length > 20) trail.pop();
      for (let i = trail.length - 1; i >= 0; i--) {
        const age = i / trail.length;
        halo(trail[i].x, trail[i].y, 38 + 24 * (1 - age), fade * (.1 + (1 - age) * .4));
      }
      halo(follower.x, follower.y, 33, fade * .44);
    ...
    function onMove(event) {
      ...
      if (Math.hypot(next.x - target.x, next.y - target.y) < 1.5) return;
      if (now - lastMove > 260) { follower = { ...next }; trail.length = 0; }
      target = next;
      lastMove = now;
